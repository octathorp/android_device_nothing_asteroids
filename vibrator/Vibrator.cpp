...
#define RICHTAP_CLICK_EFFECT 12298
#define RICHTAP_TICK_EFFECT 12291
...
ndk::ScopedAStatus Vibrator::perform(Effect ef, EffectStrength es,
                                     const std::shared_ptr<IVibratorCallback>& callback,
                                     int32_t* _aidl_return) {
    int32_t effect;
    int32_t strength;

    if (ef < Effect::CLICK || ef > Effect::HEAVY_CLICK)
        ALOGI("AAC perform effect error: %u\n", ef);
    ...
    switch (ef) {
        case Effect::CLICK:
            effect = RICHTAP_CLICK_EFFECT;
            break;
        case Effect::TICK:
            effect = RICHTAP_TICK_EFFECT;
            break;
        default:
            effect = RICHTAP_CLICK_EFFECT;
    }
